...
- **腾讯云 COS 图床**：国内机房公网直链，CSDN 转存 100% 成功；同图自动去重
- **GitHub 图床（免费）**：图片传到你的公开仓库，适合备份/海外平台
...
### 模式二：腾讯云 COS（CSDN/掘金等平台推荐，最稳）

| | |
|---|---|
| **原理** | 复制时把图片上传到你的 COS 存储桶（公有读私有写），md 里替换为 COS 公网直链（`https://桶名.cos.地域.myqcloud.com/...`） |
| **优点** | 国内机房 + CDN，CSDN 转存 100% 成功；新用户 6 个月免费 50GB，之后每月几毛钱 |
| **去重** | 文件名取图片内容 SHA256 前 16 位 —— 同图跨会话只传一次 |
| **注意** | 需实名认证；存储桶权限必须选「公有读私有写」 |

**配置步骤**（只需一次）：

1. 注册腾讯云（cloud.tencent.com）并完成个人实名认证
2. COS 控制台创建存储桶：地域选 `ap-guangzhou`，权限选**公有读私有写**
3. 访问 CAM 密钥页面（console.cloud.tencent.com/cam/capi）新建密钥，得到 SecretId / SecretKey
4. Mopai ⚙ 设置 → 选择「腾讯云 COS」→ 填入 SecretId、SecretKey、存储桶名（含 APPID 后缀）、地域 → 保存

### 模式三：GitHub 图床（免费，但国内 CSDN 场景受限）

| | |
|---|---|
| **原理** | 复制时把图片上传到你的 GitHub **公开仓库**，md 里的图片替换为 jsDelivr CDN 链接（`cdn.jsdelivr.net/gh/...`） |
| **优点** | 免费、永久、图片在 GitHub 有版本管理备份；同图跨会话只传一次 |
| **局限** | jsDelivr 已停止代理 GitHub 文件（301 到 raw.githubusercontent.com），国内网络访问 raw 不通 → **CSDN 等国内平台转存会失败**。适合海外网络环境或仅作图片备份 |
...
| | |
|---|---|
| **原理** | 复制时逐张上传到公共图床 `s.ee`（SM.MS 官方迁移目标，API 兼容），替换为公网直链 |
| **注意** | 官方声明所有套餐均付费（免费仅对老 SM.MS 迁移用户）；不推荐新用户使用 |

### 怎么切换

⚙ 设置面板 → 图床模式选择：**本地 HTTP（默认）** / **腾讯云 COS** / **GitHub 图床** / **s.ee**。选择对应模式后填写所需配置。
...
|----------|----------|
| 微信公众号 | 本地 HTTP（默认） |
| CSDN / 掘金 / 知乎（复制 Markdown 源码） | 腾讯云 COS |
| 海外平台 / 图片永久备份 | GitHub 图床 |
