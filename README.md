...
### 模式二：GitHub 图床（免费，但国内 CSDN 场景受限）
...
### 模式三：s.ee 图床（有免费版，国内 CSDN 推荐）

| | |
|---|---|
| **原理** | 复制时逐张上传到公共图床 `s.ee`（SM.MS 官方迁移目标，API 兼容），替换为公网直链 |
| **优点** | 免费计划可用（5GB 容量、每天 200 次上传）；`s.ee` 国内可访问、无防盗链，**CSDN 转存稳定**；图片有独立公网直链 |
| **注意** | 需注册 s.ee（邮箱即可）并在「开发者设置 → API Token」生成 Key；付费计划才有更大额度/自定义域名 |

**获取 s.ee API Key**：注册 https://s.ee → 登录 → 工具/开发者设置 → API Token → 生成。
...
|----------|----------|
| 微信公众号 | 本地 HTTP（默认） |
| CSDN / 掘金 / 知乎（复制 Markdown 源码） | s.ee 图床（免费版） |
